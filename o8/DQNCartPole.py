...
class DQNCartPoolAgent(DQNAgent):
    def __init__(self, state_size, action_size, buffer_size=10000, batch_size=64, gamma=0.99, lr=1e-3, tau=1e-3,
        ...
        super().__init__(state_size, action_size, buffer_size, batch_size,
                         gamma, lr, tau, epsilon, epsilon_min, epsilon_decay)
    ...
    def learn(self, experiences):
        ...
        # Assuming cart position is at index 0 of the state array
        cart_positions = states[:, 0]

        # Apply the penalty based on the cart position
        # Penalize based on distance from center (Cart Position = 0)
        penalties = abs(cart_positions * 1.0)
        rewards = rewards - penalties  # Subtract the penalty from the rewards

        # Convert the experiences to PyTorch tensors
        states = torch.FloatTensor(states)
        next_states = torch.FloatTensor(next_states)
        # Add an extra dimension to rewards for batch processing
        rewards = torch.FloatTensor(rewards).unsqueeze(1)
        dones = torch.FloatTensor(dones).unsqueeze(1)  # Same for dones

        # Actions need to be long tensors and have an extra dimension
